"""
Email service: send invitation emails via SMTP.
"""
...
async def send_invitation_email(
    ...
    if not settings.smtp_host:
        invite_url = f"{settings.app_base_url}/invite?token={token}&org={org_name}"
        logger.warning("SMTP not configured — invitation link: %s", invite_url)
        return False
