...
def _resolve_model_name_or_path(model_name_or_path: str) -> str:
    """
    Resolve local model paths after the backend moved under backend/.

    Hugging Face model IDs are returned unchanged. Relative paths such as
    ./training/models/... are allowed to keep working by checking both the
    repository root and backend/ as possible bases.
    """
    ...
    if not path.is_absolute():
        clean = Path(raw[2:] if raw.startswith("./") else raw)
        candidates.extend(
            [
                project_root / clean,
                backend_dir / clean,
                project_root / "backend" / clean,
            ]
        )
    ...
    for candidate in candidates:
        if candidate.exists():
            ...
            logger.info("Resolved embedding model path %s -> %s", raw, resolved)
            return resolved
