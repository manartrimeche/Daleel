...
async def get_qa_daily_counts(db: Any, days: int = 30) -> list[dict]:
    ...
    rows = await db["qa_feedback"].aggregate(pipeline).to_list(length=None)
...
async def get_compliance_coverage(db: Any) -> list[dict]:
    ...
    """
    profiles = await db["company_profiles"].find({}, {"_id": 0, "id": 1, "name": 1}).to_list(length=None)
    ...
    for profile in profiles:
        applicable = await db["exigence_applicabilities"].count_documents({
